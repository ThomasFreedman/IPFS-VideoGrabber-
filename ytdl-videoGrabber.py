...
# CONSTANTS
#
EMAIL_SERVR     = ["your_email_SMTP_server_address", port #]
EMAIL_LOGIN     = ["your_email_user", "login password for that user"]
EMAIL_LIST      = "comma separated list of email addresses to send results"
EMAIL_URLS      = EMAIL_LIST  # A separate list to send titles and URLs to
DOWNLOAD_IP     = "111.222.333.444"  # The IP address to use for downloads
EMAIL_SUB1      = 'Video Grabber email subject for primary results'
EMAIL_SUB2      = 'Subject for URL emails'
EMAIL_FROM      = "originator email address"
...
Config {
    "Comment":   ["bla", "bla"]  Unreferenced - for comments inside config file
...
# makes a 2nd attempt to insert the row with an alternate list of columns. If
# the failure occurs on the second attempt the failure is indicated by a None
# return value and raising the SQLite3 exception. This function creates a new
# minimal list of columns (metadata fields).  The  ytdl extractors vary as to
# the format of the metadata they produce, it is not totally normalized, thus
# this function may require updates.
def addRow(conn, cols, gvmjList):
    try:
        row = addRow2db(conn, cols, gvmjList)   # Attempt number one...

    # On failure fallback to the following metadata columns for this video.
    except Exception as e:
        newCols = ["_filename","webpage_url","duration","upload_date","width",
                   "height","id","extractor","ext","vcodec","acodec","title" ]

        # Fix "bad" metadata by creating key/value pairs for missing data.
        newDictionary = {}
        for col in newCols:
            if col in gvmjList[3].keys():
                newDictionary[col] = gvmjList[3][col]
            else: newDictionary[col] = "generic-unknown-value"

        # Try again. Any exception this time will propagate upstream
        row = addRow2db(conn, newCols, newDictionary)
...
def processGrupeResults(conn, cols, urls, grupe, eLog):
    ...
    if downloads > 0:
        for dat in SQLrows2Add:  # grp, vhash, mhash, json
            try:
...
def processVideo(file):
    ...
    try:
        ...
        # If all processed OK, save the info to be added to the SQLite database
        if mHash != None and vHash != None and len(jFlat) > 0:
            SQLrows2Add.append([grp, vHash, mHash, jFlat])
...
def displaySummary(conn):
    ...
    if len(rows) > 0:
        ...
        print(args)
        for col in rows:
            args = (col['grupe'], col['title'], col['vhash'])
...
###############################################################################
# main is only called if this file is a script not if object class definition #
# If this code is useful as a class it will be easy to make it one. I see no  #
# point to do so at this time.                                                #
###############################################################################

...
#
# Program to obtain json metadata files for videos.
#
...
Targets = {
    ...
    'tragdyHope': ['https://www.youtube.com/user/TragedyandHopeMag/videos'],
    'anarchast':  ['https://www.youtube.com/user/TheAnarchast/videos'],
    'beyondVeil': ['https://www.youtube.com/user/truthfrequency/videos'],
...
#############################################################################
#                                                                           #
# The main download loop. youtube-dl takes care of downloading the metadata #
# for all of the videos.                                                    #
#                                                                           #
#############################################################################
for grupe in Targets:
    url = Targets[grupe][0]
    if not os.path.isdir(DLBASE):
        os.mkdir(DLBASE)
    if not os.path.isdir(DLBASE + grupe):
        os.mkdir(DLBASE + grupe)
    where = "cd " + DLBASE + grupe
    to = "' > " + grupe + ".json"
    os.system(where + "; youtube-dl -j -v --flat-playlist '" + url + to)
...
#                                                                           #
#############################################################################
if not os.path.isdir(DLBASE): os.mkdir(DLBASE)
